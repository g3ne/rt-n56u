...
chnbadip_file="/etc/storage/chinadns/chinadns_iplist.txt"
...
func_start(){
	start-stop-daemon -S -b -x chinadns -- -m -l "$chnbadip_file" -c "$chnroute_file" -b "$bind_address" -p "$bind_port" -s "$server" "$(get_arg_filter)"
}
